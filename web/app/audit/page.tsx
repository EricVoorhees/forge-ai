    ...
    <div className="min-h-screen bg-[#09090b] text-white">
      ...
      <section className="relative pt-32 pb-24 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight mb-4 md:mb-6 max-w-3xl mx-auto leading-[1.1]">
              AI-Powered Code
              <br />
              <span className="bg-gradient-to-r from-orange-400 to-amber-400 bg-clip-text text-transparent">Security Analysis</span>
            </h1>
            
            <p className="text-[#a1a1aa] text-base md:text-lg lg:text-xl max-w-2xl mx-auto mb-8 md:mb-10 leading-relaxed px-4">
              Detect vulnerabilities, security flaws, and code quality issues before they reach production.
            </p>
            
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4">
              <button
                onClick={scrollToTool}
                className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 bg-white text-black font-semibold rounded-xl hover:bg-zinc-200 transition-colors"
              >
                Start Audit
              </button>
              <Link
                href="/audit-docs"
                className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 bg-white/5 border border-white/10 text-white font-medium rounded-xl hover:bg-white/10 transition-colors text-center"
              >
                Documentation
              </Link>
            </div>
          ...
          <div className="flex items-center justify-center gap-6 sm:gap-10 md:gap-16 lg:gap-20">
            <div className="text-center">
              <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-1 tabular-nums">{stat1}</div>
              <div className="text-[#71717a] text-xs sm:text-sm">Vuln Types</div>
            </div>
            <div className="text-center">
              <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-1 tabular-nums">{stat2}</div>
              <div className="text-[#71717a] text-xs sm:text-sm">Scan Time</div>
            </div>
            <div className="text-center">
              <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-1 tabular-nums">{stat3}</div>
              <div className="text-[#71717a] text-xs sm:text-sm">Detection</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="audit-tool" className="py-24 border-t border-white/[0.06]">
        <div className="max-w-5xl mx-auto px-6">
          ...
          {/* Input Method Tabs - Scrollable on mobile */}
          <div className="overflow-x-auto pb-2 mb-6 -mx-6 px-6">
            <div className="flex items-center gap-1 p-1 bg-[#18181b] rounded-xl w-fit mx-auto min-w-max">
              {[
                { id: "paste", label: "Paste", labelFull: "Paste Code", icon: "M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" },
                { id: "upload", label: "Upload", labelFull: "Upload Files", icon: "M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" },
                { id: "github", label: "GitHub", labelFull: "GitHub", icon: "M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" },
                { id: "cicd", label: "CI/CD", labelFull: "CI/CD", icon: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" },
              ].map((method) => (
                <button
                  key={method.id}
                ...
                >
                  <svg className="w-3.5 h-3.5 md:w-4 md:h-4" fill={method.id === "github" ? "currentColor" : "none"} stroke={method.id === "github" ? "none" : "currentColor"} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={method.icon} />
                  </svg>
                  <span className="md:hidden">{method.label}</span>
                  <span className="hidden md:inline">{method.labelFull}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      <section className="py-16 md:py-24 border-t border-white/[0.06]">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <div className="flex items-center justify-center gap-2 mb-4 md:mb-6">
            <Image src="/forge-logo.png" alt="F" width={32} height={32} className="md:w-10 md:h-10" />
            <span className="text-xl md:text-2xl font-semibold">Audit</span>
          </div>
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4 md:mb-6">Ready to Secure Your Code?</h2>
          <p className="text-[#a1a1aa] text-base md:text-lg mb-8 md:mb-10 max-w-xl mx-auto">
            Join thousands of developers who trust Forge Audit to keep their applications secure.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4">
            <Link
              href="/dashboard"
              className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 bg-white text-black font-semibold rounded-xl hover:bg-zinc-200 transition-colors text-center"
            >
              Get Started Free
            </Link>
            <Link
